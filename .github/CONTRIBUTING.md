Contributing to AlyCompiler!
======================

Thanks for your interest in contributing to AlyCompiler! :tada: We love getting [pull requests](https://www.quora.com/GitHub-What-is-a-pull-request) for bugfixes and contributions of our community to keep AlyCompiler growing.
...
As NorthernL1ghts write documentation for AlyCompiler in English, the devs ask to keep the code base, issues and pull requests in English only.
...
You've created a new fix or feature for AlyCompiler. Awesome!

1. If you haven't already, create a fork of the AlyCompiler repository.
...
4. Give us a moment. AlyCompiler is maintained by myself, in my limited free time, so it may take me a bit to review your request. Bug fixes should be merged in directly, while features usually require NorthernL1ghts approval with or without it mentioned in one (or more).
...
- Naming convention:
  - For functions we use snake case: **`function_name`**.
  - For (scoped) variables and function parameters we use snake case: **`variable_name`** and **`parameter_name`**.

  - For class names we use pascal case: **`ClassName`**.

  - For macros we use snake case: **`MACRO_NAME`**.
    - If it is specifically related to AlyCompiler, we add the 'HZ_' prefix: **`MACRO_NAME`**.
    - If there is a macro for the application and for the engine, we add an additional 'CORE_' prefix to the engine macro:  **`CORE_MACRO_NAME`**.
...
[issue-tracker]: https://github.com/NorthernL1ghts/AlyCompiler/issues
[submit-pr]: https://github.com/NorthernL1ghts/AlyCompiler/pulls
