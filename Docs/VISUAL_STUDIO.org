...
It should be noted that Aly compiler likely does not compile with MSVC due to the use of ~typeof~, a C23 language feature currently only supported by GCC and Clang. Luckily, Visual Studio can work with =clang-cl=, a tool that mimics MSVC on the command line but is just Clang in a trenchcoat.
