...
;; Maintainer: NorthernL1ghts
;; Version: 0.0.1
;; Keywords   : Aly languages tree-sitter
...
(defvar int-ts-mode--keywords
  ...
  "Aly keywords for tree-sitter font-locking.")
...
(defvar int-ts-mode--operators
  ...
  "Aly operators for tree-sitter font-locking.")
...
(defvar int-ts-mode--delimiters
  '("(" ")"
    ...
    "{" "}"
    ";")
  "Aly delimiters for tree-sitter font-locking.")
...
in Aly source code will be indented."
  :group 'int-ts-mode)

(defun int-ts--indent-line ()
  "Indent a line in Aly."
  (let ((indent)
        (boi-predicate)
        (should-move-eol)
        (point (point)))
...
(define-derived-mode int-ts-mode--base-mode prog-mode "int"
  "Major mode for editing Aly, powered by tree-sitter. "
